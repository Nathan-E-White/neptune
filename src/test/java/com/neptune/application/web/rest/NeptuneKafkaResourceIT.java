...
import com.neptune.application.config.KafkaProperties;
...
class NeptuneKafkaResourceIT {
    ...
    void setup() {
        ...
        NeptuneKafkaResource kafkaResource = new NeptuneKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/neptune-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/neptune-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
